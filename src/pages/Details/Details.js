...
function Details() {
  ...
  const storedDefinitions = JSON.parse(
    localStorage.getItem("wordsDefinitions")
  );
  const presentDefinition = storedDefinitions.filter(
    (definition, index) => definition.word === presentWord
  );
  return (
    <div>
      <Layout>
